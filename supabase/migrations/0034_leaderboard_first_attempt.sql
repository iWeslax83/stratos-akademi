-- GÜVENLİK/BÜTÜNLÜK: quiz puanı şişirilebiliyordu.
--
-- submitQuiz sonuçla birlikte DOĞRU CEVAPLARI döndürür (öğrenme amaçlı, sonuç ekranında
-- gösterilir) ve deneme sayısı sınırsız. Liderlik quiz başına max(puan) aldığı için üye
-- bilerek boş bir deneme gönderip cevapları öğrenip tekrar gönderiyor ve 100 alıyordu.
-- Özel bir araç bile gerekmiyordu: arayüzdeki "tekrar dene" yetiyordu.
--
-- Düzeltme: tekrar denemeler öğrenme için serbest kalır, ama PUANA İLK DENEME sayılır.
-- (max(puan) → ilk created_at'in puanı.)
