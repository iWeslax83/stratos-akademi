      ...
      <div className="mb-4 flex gap-2">
        {SEKMELER.map((s) => (
          <Link
            ...
            className={clsx(
              "rounded-full px-3.5 py-1.5 text-2sm font-semibold",
              aralik === s.key
                ? "bg-navy text-white dark:bg-accent dark:text-navy"
                : "bg-tint text-muted hover:text-fg",
            )}
          >
            {s.label}
          </Link>
          ...
          rows.map((r) => (
            <div
              ...
              className={clsx(
                ...
                r.userId === user!.id && "-mx-3 rounded-xl border-b-0 bg-accent-wash px-3",
              )}
            >
              <span
                className={clsx(
                  "w-7 text-center font-display text-sm font-extrabold",
                  r.sira <= 3 ? "text-accent-fg" : "text-muted",
                )}
              >
                {r.sira}
              </span>
              ...
              <span className="shrink-0 font-display text-sm font-bold text-accent-fg">
                {r.puan} puan
              </span>
            </div>
          ))
