        ...
        {ADMIN_LINKS.map((l) => {
          ...
          return (
            <Link
              ...
              onClick={onClose}
              className={clsx(
                "block rounded-lg px-3 py-2 text-sm font-semibold text-accent-ink dark:text-accent",
                active ? "bg-accent-soft dark:bg-accent-dark" : "hover:bg-black/5 dark:hover:bg-white/10",
              )}
            >
              {l.label}
            </Link>
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        className="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-[13px] font-semibold text-accent-ink hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent dark:text-accent"
      >
