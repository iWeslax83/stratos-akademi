  ...
  return (
    <div
      className={clsx(
        ...
        me && "-mx-2.5 rounded-xl border-b-0 bg-accent-wash px-2.5",
      )}
    >
      <span
        className={clsx(
          "w-6 text-center font-display text-sm font-extrabold tabular-nums",
          row.sira <= 3 ? "text-accent-fg" : "text-muted",
        )}
      >
        {row.sira}
      </span>
  ...
  return (
    <div className="p-6">
      <div className="mb-3.5 flex items-center justify-between">
        <h2 className="font-display text-lg font-bold text-fg">Liderlik</h2>
        <Link href="/liderlik" className="text-2sm font-semibold text-muted">
          Tablo →
        </Link>
      </div>
      {rows.length === 0 ? (
